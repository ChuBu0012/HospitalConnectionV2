              ...
              {menuItems.map((menuItem, index) => (
                <li key={index}>
                  {menuItem.dropdownItems ? (
                    ...
                    <Link href="#" onClick={() => {
                      menuItem.text === "ออกจากระบบ" ? signOut({ callbackUrl: '/' }) : "";
                    }} className={`${menuItem.text === "ออกจากระบบ" ? session ? "block" : "hidden" : ""} block py-2 pl-3 pr-4 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-black`}>
                      {menuItem.text}
                    </Link>
                  )}
                  {menuItem.dropdownItems && (
