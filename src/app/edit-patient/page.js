...
import {
    ...
    setAllergic, setTel, setEmail, setDefaults, setFormdata
} from '@/redux/setform';
...
function EditForm({ id, loading }) {
    ...
    const handleSubmit = () => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                updateData(id, { _id: id, ...formState }).then((res) => {
                    setloading_in(true);
                    Swal.fire({
                        icon: 'success',
                        title: 'แก้ไขสำเร็จ',
            ...
            if (result.isConfirmed) {
                deleteUser(id).then((res) => {
                    ...
                    Swal.fire({
                        ...
                        if (res.isConfirmed) {
                            router.push(`/${session?.user?.name?.role === 'Doctor' ? 'doctor' : "user"}`)
                        }
                    })
                }).catch((err) => {
                    setloading_in(true)
    ...
    return (
        loading || loading_in ? <div className='flexitemcenter justify-center'>
            ...
            <form onSubmit={(e) => {
                ...
                <div className='flexitemcenter'>
                    ...
                    <div className='whitespace-nowrap'>
                        <input disabled={session?.user?.name?.role === "Doctor" ? false : true} id="idcard" value={idcard} onInput={(e) => dispatch(setIdcard(e.target.value))} type="text" className=' outline-none ml-4 rounded bg-bginput px-2 py-1 border' />
                ...
                <div className='mt-4 flexitemcenter gap-x-8 gap-y-4 flex-wrap'>
                    <div className='flexitemcenter  w-[30%] justify-between'>
                        ...
                        <div className='whitespace-nowrap'>
                            ...
                            <select disabled={session?.user?.name?.role === "Doctor" ? false : true} id="prefix" value={prefix} onInput={(e) => dispatch(setPrefix(e.target.value))} className='border-2 rounded p-1 ml-4'>
                                <option value="--" >--</option>
                    ...
                    <div className='flexitemcenter lg:ml-8 flex-1 justify-between'>
                        ...
                        <div className='whitespace-nowrap'>
                            <input disabled={session?.user?.name?.role === "Doctor" ? false : true} id="name" value={name} onInput={(e) => dispatch(setName(e.target.value))} type="text" className=' outline-none ml-4 rounded bg-bginput px-2 py-1 border' />
                            <label className='text-2xl  text-red-500 ml-2'>*</label>
                        </div>
                    </div>
                ...
                <div className='mt-4 flexitemcenter gap-x-8 gap-y-4 flex-wrap'>
                    ...
                    <div className='flexitemcenter  w-[30%] justify-between'>
                        ...
                        <div className='whitespace-nowrapz'>
                            <select disabled={session?.user?.name?.role === "Doctor" ? false : true} id="sex" value={sex} onInput={(e) => dispatch(setSex(e.target.value))} className='border-2 rounded p-1 min-w-[84px]'>
                                <option value="--" >--</option>
                ...
                {/* วัน เดือน ปีเกิด */}
                <div className='mt-4'>
                    <Brithday checkdoctor={session?.user?.name?.role === "Doctor" ? false : true} />
                </div>
                {/* ที่อยู่ ตำบล อำเภอ จังหวัด */}
                <Address checkdoctor={session?.user?.name?.role === "Doctor" ? false : true} />
                ...
                <div className='mt-4 flexitemcenter  gap-x-8 gap-y-4 flex-wrap'>
                    ...
                    <div className='flexitemcenter justify-between'>
                        ...
                        <input disabled={session?.user?.name?.role === "Doctor" ? false : true} value={blood} onInput={(e) => dispatch(setBlood(e.target.value))} type="text" className=' outline-none lg:ml-4 rounded bg-bginput px-2 py-1' />
                    ...
                    <div className='flexitemcenter w-3/4 justify-between'>
                        ...
                        <input disabled={session?.user?.name?.role === "Doctor" ? false : true} value={allergic} onInput={(e) => dispatch(setAllergic(e.target.value))} type="text" className=' outline-none lg:ml-4 rounded bg-bginput px-2 py-1' />
                    ...
                    <div className='flexitemcenter w-3/4 justify-between relative'>
                        ...
                        <div className='relative'>
                            <input id='tel' disabled={session?.user?.name?.role === "Doctor" ? false : true} value={tel} onInput={(e) => dispatch(setTel(e.target.value))} type="text" className=' outline-none lg:ml-4 rounded bg-bginput px-2 py-1' />
                    ...
                    <div className='flexitemcenter w-3/4 justify-between'>
                        ...
                        <input disabled={session?.user?.name?.role === "Doctor" ? false : true} value={email} type="text" onInput={(e) => dispatch(setEmail(e.target.value))} className=' outline-none lg:ml-4 rounded bg-bginput px-2 py-1' />

                    </div>
                </div>
                {
                    session?.user?.name?.role === 'Doctor' && session && <div className='flex'>
                        ...
                        <button type='button' onClick={() => deleteData()} className="flexitemcenter justify-center cursor-pointer mt-8 mb-4 text-center bg-red-700 hover:bg-red-600 text-white w-2/5 min-w-[171px]  m-auto font-bold py-2 px-4 rounded">
                            ลบข้อมูล
                        </button>
                    </div>
                }
